...
public class PlaybackStateHelper {
    ...
    public static long getAdjustedSeekPos(PlaybackState state) {
        long now = SystemClock.elapsedRealtime();
        long then = state.getLastPositionUpdateTime();
        long seekPos = state.getPosition();
        Timber.d("getAdjustedSeekPos now=%d, then=%d, pos=%d, adj=%d",
    ...
    public static void logState(PlaybackState state) {
        if (state == null) return;
        Timber.v("PlaybackState %s position=%d updated=%d, bufferedpos=%d, speed=%.02f",
                stringifyState(state.getState()), state.getPosition(),
                state.getLastPositionUpdateTime(), state.getBufferedPosition(),
