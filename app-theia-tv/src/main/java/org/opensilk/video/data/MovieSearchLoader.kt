    ...
    override fun makeObservable(): Observable<MediaBrowser.MediaItem> {
        return Observable.create {
            mContentResolver.query(mClient.uris().movieSearch(),
                ...
                if (!c.moveToFirst()) {
                    return@use Observable.empty<MediaBrowser.MediaItem>()
                }
                ...
                sb.append(")")
                return@use makeMovieObservable(sb.toString())
            }
        }
    }
    ...
    internal fun makeMovieObservable(selection: String): Observable<MediaBrowser.MediaItem> {
        return Observable.create {
            mContentResolver.query(mClient.uris().media(),
                ...
                val movies = ArrayList<MediaBrowser.MediaItem>(c2.count)
                do {
                    movies.add(mClient.buildMedia(c2))
                } while (c2.moveToNext())
                return@use Observable.from(movies)
            }
        }
    }
}
