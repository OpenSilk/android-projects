...
public class VLCOptions {
    ...
    public static ArrayList<String> getLibOptions(final Context context) {
        final VideoAppPreferences pref = VideoApp.getPreferences(context);
        ...
        String chroma = pref.getString("chroma_format", context.getString(R.string.chroma_format_default));
    ...
    public static void setMediaOptions(Media media, Context context, int flags) {
        boolean noHardwareAcceleration = (flags & MediaFlags.MEDIA_NO_HWACCEL) != 0;
        boolean noVideo = (flags & MediaFlags.MEDIA_NO_VIDEO) != 0;
        final boolean paused = (flags & MediaFlags.MEDIA_PAUSED) != 0;
    ...
    public static MediaPlayer.Equalizer getEqualizer(Context context) {
        ...
        if (pref.getBoolean("equalizer_enabled", false)) {
            final float[] bands = pref.getFloatArray("equalizer_values");
    ...
    public static void setEqualizer(Context context, MediaPlayer.Equalizer eq, int preset) {
        final VideoAppPreferences pref = VideoApp.getPreferences(context);
        if (eq != null) {
            pref.putBoolean("equalizer_enabled", true);
            final int bandCount = MediaPlayer.Equalizer.getBandCount();
            ...
            pref.putFloatArray("equalizer_values", bands);
            pref.putInt("equalizer_preset", preset);
        } else {
            pref.putBoolean("equalizer_enabled", false);
        }
    }
}
