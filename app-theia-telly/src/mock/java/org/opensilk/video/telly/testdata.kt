...
fun testUpnpDeviceItem(): MediaBrowser.MediaItem {
    ...
    mediaExtras.mediaId = MediaRef(UPNP_DEVICE, UpnpDeviceId("upnpcds-1")).toJson()
...
fun testUpnpFolderItem(): MediaBrowser.MediaItem {
    val mediaExtras = MediaMeta()
    mediaExtras.mediaId = MediaRef(UPNP_FOLDER, UpnpFolderId("upnpcds-1", "1")).toJson()
    mediaExtras.parentMediaId = MediaRef(UPNP_FOLDER, UpnpFolderId("upnpcds-1", "0")).toJson()
...
fun testUpnpFolderItemList(): List<MediaBrowser.MediaItem> {
    ...
    for (ii in 1..10) {
        val mediaExtras = MediaMeta()
        mediaExtras.mediaId = MediaRef(UPNP_FOLDER, UpnpFolderId("upnpcds-1", "$ii")).toJson()
        mediaExtras.parentMediaId = MediaRef(UPNP_FOLDER, UpnpFolderId("upnpcds-1", "0")).toJson()
...
fun testUpnpVideoItem(): MediaBrowser.MediaItem {
    val mediaExtras = MediaMeta()
    mediaExtras.mediaId = MediaRef(UPNP_VIDEO, UpnpVideoId("upnpcds-1", "video1")).toJson()
    mediaExtras.parentMediaId = MediaRef(UPNP_FOLDER, UpnpFolderId("upnpcds-1", "1")).toJson()
