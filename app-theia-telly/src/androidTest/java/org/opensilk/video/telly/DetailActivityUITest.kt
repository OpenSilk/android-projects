...
class DetailActivityUITest {
    ...
    val mItem = upnpVideo_folder_1_no_association().toMediaItem()
    @Rule @JvmField
    val mActivityRule = object : ActivityTestRule<DetailActivity>(DetailActivity::class.java) {
        override fun getActivityIntent(): Intent {
            return Intent().putExtra(EXTRA_MEDIAID, mItem.mediaId)
        }
    }

    @Test
    fun activity_starts() {
        assertThat(mActivityRule.activity.intent.getStringExtra(EXTRA_MEDIAID)).isEqualTo(mItem.mediaId)
